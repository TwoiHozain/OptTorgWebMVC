...
public partial class StorageEmployees
{
    ...
    public virtual ICollection<Sending> Sending1 { get; set; } = new List<Sending>();
    
    public virtual ICollection<Sending> Sending2 { get; set; } = new List<Sending>();
    ...
    public static List<StorageEmployees> GetGlavBuh(long id)
    {
        var db = new OptTorgDBContext();
        var p = db.StorageEmployees
            .Include(x => x.EmployeeNavigation)
        ...
        return p.Where(x => x.Active == true && x.EmployeeNavigation.PositionId == 22).ToList();
    }
    ...
    {
        var db = new OptTorgDBContext();
        var strg = db.StorageEmployees.FirstOrDefault(x => x.IdSe == id);
